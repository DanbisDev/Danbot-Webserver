...
def add_wrapup_player_clog(rsn, itemName, itemQuantity, itemPrice):
    with connect() as conn:
        ...
        if cursor.rowcount == 0:
            ...
            cursor.execute('''
                UPDATE wrapup_players
            SET personal_collection_logs = personal_collection_logs + 1
            WHERE username = %s
            ''', (rsn, ))
...
def add_wrapup_player_slayer_task(username, task_name, monsters_killed):
    with connect() as conn:
        ...
        cursor.execute("""
            ...
            DO UPDATE SET
                tasks_completed = wrapup_slayer_tasks.tasks_completed + 1,
                monsters_killed = wrapup_slayer_tasks.monsters_killed + EXCLUDED.monsters_killed
